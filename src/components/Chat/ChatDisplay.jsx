  ...
  function ChatDisplay({ scrollToBottomTrigger }) {
    ...
    const isFirstConnection = useRef(true); // Track first WebSocket connection
    const hasScrolledToBottom = useRef(false); // Track if scroll to bottom achieved
    ...
    const scrollToBottom = useCallback(() => {
      if (messageEndRef.current) {
        ...
        if (!hasScrolledToBottom.current) {
          console.log('Scroll to bottom achieved');
    ...
    const setupWebSocket = useCallback(() => {
      ...
      try {
        ...
        socket.onopen = () => {
          ...
          // Scroll on first connection if not already scrolled
          if (isFirstConnection.current && !hasScrolledToBottom.current) {
            console.log('First WebSocket connection established');
            scrollToBottom();
            isFirstConnection.current = false;
          } else {
            console.log('WebSocket reconnected, skipping scroll');
          }
            ...
            if (data.type === 'messages') {
              ...
              setMessages((prev) => {
                const existingIds = new Set(
                  prev.map((msg) => msg.message_id || msg.temp_id)
                );
                const filtered = newMessages.filter(
                  (msg) => !existingIds.has(msg.message_id || msg.temp_id)
                );
              ...
              setMessages((prev) =>
                prev.map((msg) =>
                  msg.status === 'sending' && msg.temp_id
                    ? { ...msg, status: 'failed' }
                    : msg
                )
              );
    ...
    }, [contextUser, formatMessage, sendPendingMessages, cleanupWebSocket, scrollToBottom]);
    ...
    // Scroll to bottom when scrollToBottomTrigger changes
    useEffect(() => {
      if (scrollToBottomTrigger && !hasScrolledToBottom.current) {
        scrollToBottom();
      }
